    ...
    if bank == 'Ипотека ВТБ':  # Отдельный вариант для ВТБ
        ...
        for i in list_of_zhk:
            df = pd.read_excel(i,
                               parse_dates=True, header=[1], usecols=[5, 9, 10, 13, 17, 18, 20, 22], index_col=2)  #Чтение ЗПИФ_ЗД в датафрейм
            try:
                a = df[(df['Способ оплаты'] == bank) | (df['Способ оплаты'] == 'Военная ипотека ' + bank.split(' ')[1])
                                       &(df['Примечание'] != 'Расторжение')][date]['Стоимость квартиры'].count()
            except KeyError:
                a = 0
            try:
                b = df[(df['Способ оплаты'] == bank) | (df['Способ оплаты'] == 'Военная ипотека ' + bank.split(' ')[1])
                                       &(df['Примечание'] != 'Расторжение')][date]['Стоимость квартиры'].sum()
            except KeyError:
                b = 0
            try:
                sales_total.append((df[df['Примечание'] != 'Расторжение'][date]['Стоимость квартиры'].count(),
                                   ...
                                   df[(df['Упрощенный способ оплаты'] == 'ипотека' )&(df['Примечание'] != 'Расторжение')][date]['Стоимость квартиры'].count(),
                                    ...
                                    a,round(b,2)))  #Создаем список кортежей следующего вида с данными без расторжений
    ...
    while True:
        try:
            ...
            if var == 1:
                ...
                var = input('>>')
                return 'Ипотека ' + var.lower().title()
            else:
